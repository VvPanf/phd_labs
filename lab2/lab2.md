...
1.	Скачать программу по генерации документов для MongoDB на языке Java (ссылка: https://github.com/VvPanf/data-generator)
...
10. Сделать один запрос на агрегацию через фреймворк pipeline (функция aggregate) в один шаг.
11. Выполнить четыре запроса на агрегацию через фреймворк pipeline (функция aggregate). Каждая агрегация должна состоять как минимум из 3-х этапов. Текстом написать, что должна выводить каждая агрегация. Представить описание запроса, сам запрос и его результаты.
12. Один интересный запрос на MapReduce (функция mapReduce), который бы не повторял простейшие функции из pipeline!!! В это задании в вашем распоряжении вся сила языка JavaScript в функциях, которые мы передаём в mapReduce, поэтому постарайтесь придумать что-то интересное!!!
