...
```bash
docker run --name my-mongo -dit -p 27017:27017 mongo:latest
```
...
Задание:
1. Выбрать предметную область из списка или придумать свою. Предметные области внутри группы не должны повторяться.
2. Установить сервер MongoDB (по желанию) и клиент Compass для работы с ним. Можно подключиться и к моему серверу.
3. Создать новую базу данных (если на своём сервере) и коллекцию в ней. В качестве названия коллекции использовать короткое слово, обозначающее вашу предметную область.
4. Придумать структуру документа, который будет хранится в коллекции. Из каких полей он будет состоять? Сделать в отчёте табличку с этой структурой: название, тип данных, описание.
    - Полей в документе должно быть не меньше 6, не считая поле _id, которое создастся автоматически.
    - Должно быть минимум одно строковое поле.
    - Должно быть минимум одно числовое поле.
    - Должно быть поле, содержащее в себе массив, либо объект, либо массив объектов.
    - Остальные поля на своё усмотрение.
5. Осуществить запросы на создание документов. Запросы делаем через mongosh, если не указано, что можно делать через GUI.
6. Создать минимум 15 документов. Информация в документах должна содержать какие-то реальные данные (не надо заполнять поля белибердой). Эти запросы можно делать через GUI.
    - Минимум один запрос на insertOne
    - Минимум один запрос на insertMany
7. Осуществить запросы на изменение данных.
    - Один запрос на updateOne с изменением значения существующего поля
    - Один запрос на updateOne с добавлением значения существующего поля
    - Один запрос на updateMany с увеличением значения числового поля
8. Осуществить запросы на поиск данных. Эти запросы можно делать через GUI.
    - Один запрос на findOne по значению поля
    - Два запроса на find c операторами запроса ($eq, $gt, $lt и т.д.)
    - Один запрос на find+проекция с логическими операторами ($and, $or)
    - Один запрос на countDocuments по условию
    - Один запрос с сортировкой
9. Осуществить запросы на удаление документов
    - Один запрос на deleteOne
    - Один запрос на deleteMany

Что должно быть в отчёте:
1. Название лабораторной: Основы работы с MongoDB.
2. Цель.
3. Задание.
4. Описание предметной области (какие поля в документе за что отвечают).
5. Текстом вставить все документы из коллекции после выполнения шага 5. (Мелким шрифтом – экономим бумагу).
6. По каждому запросу: 
    - Текстовое описание запроса: что хотим с помощью него сделать
    - Сам запрос
    - Скрин с результатом его выполнения
7. Вывод о том, чему научились в лабораторной работе.
 